...
export const RemoteSignatureView: React.FC<RemoteSignatureViewProps> = ({ ticketParam, usuarioNombre }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await updateRemoteSignature(ticketParam, signatureDataUrl);
      if (res.success) {
        setSubmitted(true);
      } else {
        setErrorMessage(res.message || 'No se pudo guardar la firma en la base de datos. Por favor reintenta.');
      }
    } catch (err: any) {
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4">
        <div className="glass-panel rounded-3xl p-8 max-w-md w-full text-center space-y-4 border border-emerald-500/30 shadow-2xl animate-in zoom-in duration-200">
          ...
          <p className="text-sm text-slate-300">
            Gracias <strong className="text-white">{usuarioNombre || 'Usuario'}</strong>. Tu firma de conformidad ha sido recibida y verificada en la base de datos.
          </p>
