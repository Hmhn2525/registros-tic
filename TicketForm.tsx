...
import { 
  ...
  CheckCircle2, Filter, Search, Building2, Smartphone, Share2, KeyRound, Copy, Check
} from 'lucide-react';
...
export const TicketForm: React.FC<TicketFormProps> = ({ onSuccess }) => {
  ...
  const [createdTicketShareUrl, setCreatedTicketShareUrl] = useState<string | null>(null);
  ...
  // Resolución de Dominio Base Inteligente
  const getBaseAppUrl = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const notasCompletas = [
      detalleActivo,
      modalidadFirma === 'remota_link' ? '[Firma Remota: Enlace generado para WhatsApp]' : '',
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {feedback && (
        <div className={`p-4 rounded-2xl text-xs font-semibold border flex flex-col gap-3 shadow-lg ${
      ...
      <div className="glass-panel rounded-3xl p-6 sm:p-8 space-y-6 shadow-2xl">
        ...
        <div className="pt-4 border-t border-slate-800/80 space-y-4">
          ...
          {modalidadFirma === 'remota_link' && (
            <div className="bg-indigo-950/40 p-4 rounded-2xl border border-indigo-500/30 space-y-2 animate-in fade-in duration-150">
          ...
          {modalidadFirma === 'remota_pin' && (
            <div className="bg-indigo-950/40 p-4 rounded-2xl border border-indigo-500/30 space-y-3 animate-in fade-in duration-150">
              <div className="flex items-start gap-2.5 text-xs text-indigo-200">
                ...
                <div>
                  <span className="font-bold text-white text-sm block">Confirmación por Código PIN</span>
                  El usuario remoto te proporciona un código telefónico de 4 dígitos.
                </div>
              </div>
